...
// below 2.0, as discussed in the hillslope length manuscript. This driver alos uses
// different channel head files for each resolution step.
//
...
//
// A usage example is:
// nice ./LH_Res_variable_channel.out /home/s0675405/Data/SC/ SC bil 4 2 1
//
// Outputs three text files and a csv file for every resolution step.
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
//
// Stuart W.D. Grieve
